...
export async function whatsappController(fastify: FastifyInstance, opts: FastifyPluginOptions) {
  ...
  fastify.put("/:whatsappId", async (
    ...
    try {
      ...
      const channel = await whatsappService.update(whatsappId, tenantId, payload);
  ...
  fastify.delete("/:whatsappId", async (request: FastifyRequest,
    reply: FastifyReply) => {
    try {
