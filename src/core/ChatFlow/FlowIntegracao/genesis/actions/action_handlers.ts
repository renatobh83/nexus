...
export async function handleRecuperar({
  ...
  try {
    ...
    const data = await RecuperarAcesso({
      integracao,
      email: emailMsg,
      dtNascimento,
    });
...
export async function handleConsulta({
  ...
  sessao.cadastro = cpf;
  const consultaAPI = await ConsultaPaciente({
    senha: "0",
    integracao,
    cpf,
  });
...
export async function handleSolicitarSenha({
  ...
  const retornoConsulta = await ConsultaPaciente({
    senha,
    integracao,
    cpf,
  });
...
export async function handleLinkCadastro({
  ...
  const link = await gerarLinkRegistro(cpfCadastro, integracao);
  await getFastifyApp().services.ticketService.updateTicket(ticket.id, {
    status: "closed",
    closedAt: new Date().getTime(),
    lastInteractionBot: new Date(),
...
export async function handleSelecionarUnidade({
  sessao,
  ticket,
  msg,
}: ActionHandlerParams) {
...
export async function handleGetPlanos({
  input,
  ticket,
  sessao,
}: ActionHandlerParams) {
...
export async function handleObsPlanoSelecionado({
  ...
  if (!ListaProcedimentoEmpresa) {
    const listaProcedimentos = await getListaProcedimento({
      integracao,
      cdPlano,
...
export async function handlePesquisaExame({ ticket }: ActionHandlerParams) {
  await setCache(REDIS_KEYS.previousStepId(ticket.id), ticket.stepChatFlow);
  return false;
}
export async function handleSelectProcedimento({
  ticket,
  input,
  sessao,
}: ActionHandlerParams) {
...
export async function handleLoopProcedimentos({
  ticket,
  sessao,
  msg,
  input,
...
export async function handleSelectPeriodo({ ticket }: ActionHandlerParams) {
  await getFastifyApp().services.ticketService.updateTicket(ticket.id, {
    botRetries: 0,
    lastInteractionBot: new Date(),
  });
...
export const handleIntervalos = async ({
  msg,
  ticket,
}: ActionHandlerParams) => {
  await getFastifyApp().services.ticketService.updateTicket(ticket.id, {
...
export async function handlePesquisaHorarios({
  ...
  const pesquisaHorarioResponse = await doAgendaSemanal({
    integracao,
...
export async function handleConfirmarAutoAgendamento({
  ...
  const horarioSelecionado = (await getCache(
    REDIS_KEYS.horarioAgendamento(cd_horario)
  )) as {
    dia: string;
    hora: string;
  };
  ...
  const horario = await doAgendaHorario({
    integracao,
...
export async function handleConcluirAgendamento({
  ...
  const cd_horario = (await getCache(
    REDIS_KEYS.horarioAgendamento(sessao.cdHorario)
  )) as any;
  const dadosDoAgendaHorario = {
  ...
  const responseConcluirAgendamento = await doAgendaPost({
    integracao,
    dadosPesquisa: dadosDoAgendaHorario,
  });
...
export async function handlePrecoExame({
  ...
  try {
    const medicos = await ListaMedicoExame({
      cdProcedimento: exame,
      integracao,
      token: sessao.dadosPaciente.ds_token,
      cdEmpresa: sessao.unidadeSelecionada,
    });
    precoExame = await PrecoExame({
      cdProcedimento: exame,
      sessao: sessao,
      integracao,
    });
